...
const Three = () => {
  ...
  const switcher = ()=>{
    ...
    gsap.to('.dot',{
      left:mode ? '5%' : '58%',
      backgroundImage:mode? 'linear-gradient(180deg,#ffcc89,#d8860b)' :'linear-gradient(180deg,#777,#3a3a3a)',
      duration:.5,
      ease:'power2.inOut'
    })
      ...
      if(!mode) {
        ...
        gsap.to(skyRef.current.material.uniforms.turbidity,{
          value:60,
          duration:2
        })
        gsap.to(skyRef.current.material.uniforms.mieCoefficient,{
          value:.05,
          duration:2
        })
      } else {
  ...
  return (
    <>
      <Html wrapperClass="switch">
        <button className=" switcher relative flex justify-between items-center border rounded-full text-white  w-[110px] h-[50px] px-[13px]" onClick={switcher}>
      ...
      <OrbitControls />
      <Sky ref={skyRef} 
      sunPosition={[-.8,.3,-.5]} // 1,0,0 for dark 
      turbidity={10}  // 60 for dark 
      mieCoefficient={.005} // .05 for dark 
